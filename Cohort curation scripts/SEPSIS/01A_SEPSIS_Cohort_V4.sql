...
select distinct 
gender_source_value,count(distinct a.person_id)
from results.Sepsis_Cohort a 
join omop_cdm.PERSON b on a.person_id=b.person_id
group by gender_source_value with rollup

select distinct
b.person_id
from results.Sepsis_Cohort a 
join omop_cdm.death b on a.person_id=b.person_id
